export const navbar = [
  {
    path: "/",
    name: "Home",
  },

  {
    path: "/About",
    name: "About us",
  },
  {
    path: "/Career",
    name: "Career",
  },
  {
    path: "/ContactUs",
    name: "Contact us",
  },
];

// ---------------------------/services/-----------------------------------------------------
export const elements = [
  {
    imgSrc: "/assets/web.jpg",
    heading: "Web Design & Developmnet",
    details:
      "A website is the identity of your Brand. We can help to create the online presence of your Business. Our websites are mobile & tablet friendly. It looks great on every device.",
  },
  {
    imgSrc: "/assets/mobile.jpg",
    heading: "Mobile Apps Development",
    details:
      "We building powerful feature-rich mobile apps. We create mobile solutions for iPhone, Android, and other platforms. Our focus on workforce automation helps our client's business.",
  },

  {
    imgSrc: "/assets/uiux.jpg",
    heading: "UI / UX Designing",
    details:
      "Today, every business needs an attractive business website. No matter what the size of your business is, you require an effective online presence and having just a website does not serve the purpose. We are providing more than this.",
  },

  {
    imgSrc: "/assets/ecommarce.jpg",
    heading: "Ecommerce Development",
    details:
      "We building powerful feature-rich mobile apps. We create mobile solutions for iPhone, Android, and other platforms. Our focus on workforce automation helps our client's business.",
  },
];

///////////////////////////////////////career-content///////////////////////////////////////////

export const careerContent = [
  {
    text: "At Zotech PVT, our professionally skilled teams are the foundation of our organization. With our 12+ years of experience backed with an 80+ mindful team, we offer proficient software development services globally. Our organizational strength lies in giving the best efforts that not only brings exposure to working on international projects but also the team work we do together.",
  },
  {
    text: "We believe in keeping a transparent approach for all the employees.",
  },
  {
    text: "We Discuss, We Explore, We Photograph, We Work, We Create, We Play.",
  },
  {
    text: "We Grind Together, We Support Each Other, We Work Together, We Fail Sometimes, But Survive Together.",
  },
  {
    text: "When We Travel Along, We recharge ourselves, We build stronger bonds, and better understandings!",
  },
  {
    text: "We believe in Vision, We believe in Harmony, We are on A Mission to provide Out-of-the-Box Solutions!",
  },
];

///////////////////////////////////////////////////career-perk-content////////////////////////////////////

///////////////////////////////////////////career-card//////////////////////////////////////////

export const cardCareer = [
  {
    imgsrc: "/assets/n.png",
    position: "Node.Js Developer",
    location: "Surat",
    experience: "3.0 + Years",
  },
  {
    imgsrc: "/assets/r.png",
    position: "React.js Developer",
    location: "Surat",
    experience: "3.0 + Years",
  },
  {
    imgsrc: "/assets/mern.jpg",
    position: "MERN stack Developer",
    location: "Surat",
    experience: "3.0 + Years",
  },
];

///////////////////////////////////////career-content///////////////////////////////////////////

///////////////////////////////////////////////////career-perk-content////////////////////////////////////

export const careerPerk = [
  {
    text: "1. Join a Team of 80+ #elspartans",
  },
  {
    text: "2. Leadership Opportunities",
  },
  {
    text: "3. 5 Days working week",
  },
  {
    text: "4. Flexible Working Hours",
  },
  {
    text: "5. Regular Employee Refreshment Activities",
  },
  {
    text: "6. Direct Exposure to Global Clients",
  },
  {
    text: "7. No Boring Projects, Interesting Projects, Amazing Experience",
  },
  {
    text: "Reboot your career with us. Join Us, Together Let’s Become a Team of 150+ Strong ELSPARTANS.",
  },
  {
    text: "info@zotech.in",
  },
];

export const testimonials = [
  {
    name: "Mike Richardson",
    des: "“ Far far away, behind the word mountains, far from the countries Vokalia and Consonantia, there live the blind texts. Separated they live in Bookmarksgrove right at the coast of the Semantics, a large language ocean. ”",
    imgsrc: "/assets/person1.jpg",
  },
  {
    name: "Charles White",
    des: "“ Far far away, behind the word mountains, far from the countries Vokalia and Consonantia, there live the blind texts. Separated they live in Bookmarksgrove right at the coast of the Semantics, a large language ocean. ”",
    imgsrc: "/assets/person2.jpg",
  },
  {
    name: "Michal Root",
    des: "“ Far far away, behind the word mountains, far from the countries Vokalia and Consonantia, there live the blind texts. Separated they live in Bookmarksgrove right at the coast of the Semantics, a large language ocean. ”",
    imgsrc: "/assets/person3.jpg",
  },
];

export const FAQ = [
  {
    id: 1,
    question: " Dedication",
    answer:
      "We believe our customer is not an interruption of our work, Customer is the main purpose of our work.",
  },
  {
    id: 2,
    question: "Consistency",
    answer:
      "Expect 100% dedication regardless of project scope or investment size.",
  },
  {
    id: 3,
    question: "Honesty",
    answer:
      "We ask our customers to excuse us for not full filling their business needs which are beyond our capabilities.",
  },
  {
    id: 4,
    question: "Relation",
    answer: "We believe in forming core relationships.",
  },
  {
    id: 5,
    question: "Experience",
    answer:
      "A team of experienced and skill designers and developers have groomed themselves in the field of web application development for more than seven years.",
  },
  {
    id: 6,
    question: "Innovation",
    answer:
      "Star IT Solution Members are blessed with only one talent and that is innovation.",
  },
  {
    id: 7,
    question: "Integrity",
    answer:
      "We are not afraid to admit when we make mistakes, nor are we afraid to tell you about your risks.",
  },
];
